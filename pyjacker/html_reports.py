    ...
    for i in range(min(df_result.shape[0],n_events)): #range(df_result.shape[0])
        ...
        with open(os.path.join(outdir,"Data/html/"+gene_name+"_"+sample+".html"),"w") as outfile:
            ...
            if ase_dir is not None:
                ...
                tmp = outfile.write("<tbody>\n")
                write_SNPs(sample,gene_full,ase_dir,outfile)
                tmp = outfile.write("</table>\n</tbody>\n")
...
"""
def rank_order_plot(df_TPM,gene,output_filename,sample=None):
    matplotlib.rcParams.update({'font.size': 20})
    plt.figure()

    values = []
    for col in df_TPM.columns: #df_TPM.columns
        values.append(df_TPM.loc[gene.gene_id,col])
    values = sorted(values)

    if sample is not None:
        colors = ["red" if x ==df_TPM.loc[gene.gene_id,sample] else "black" for x in values]
    else:
